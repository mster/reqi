...
const { ReqiError, header, protocol } = require('./util')
...
class ClientRequest {
  ...
  async get (opts) {
    return new Promise((resolve, reject) => {
      ...
      const protocolObject = protocol(options.uri)

      protocolObject.get(options.uri, (res) => {
  ...
  async put (opts, data) {
    return new Promise((resolve, reject) => {
      ...
      const request = protocolObject.request(options, (res) => {
        res.setEncoding('utf8')
        const { statusCode } = res
      ...
      if (options.parseJSON) request.write(JSON.stringify(data))
  ...
  async head (opts) {
    return new Promise((resolve, reject) => {
      if (!opts || !opts.uri) {
        reject(new ReqiError('Error: HEAD/ options.url is required.'), opts)
      }
